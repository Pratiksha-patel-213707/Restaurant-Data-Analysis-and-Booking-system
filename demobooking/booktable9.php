...
if(isset($_GET['date'])){
    $date = $_GET['date'];
    $stmt = $mysqli->prepare("select * FROM  booking_bhopal_1_9 where date = ?");
...
if(isset($_POST['submit'])){
    ...
    if($stmt->execute()){
        ...
        // }
        {
            $stmt = $mysqli->prepare("INSERT INTO booking_bhopal_1_9 (name, email,phone,name2,name3,name4,name5,name6,name7,name8,name9 , date,timeslot,location,amount,table_size) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)");
            $stmt->bind_param('ssssssssssssssss', $name,  $email,$phone,$name2,$name3,$name4,$name5,$name6,$name7,$name8,$name9, $date,$timeslot,$book,$amount,$table_size);
            $stmt->execute();

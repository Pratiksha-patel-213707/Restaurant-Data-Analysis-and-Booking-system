...
if(isset($_GET['date'])){
    $date = $_GET['date'];
    $stmt = $mysqli->prepare("select * FROM  booking_bhopal_1_6 where date = ?");
    $stmt->bind_param('s', $date);
    $bookings = array();
    if($stmt->execute()){
        $result = $stmt->get_result();
    ...
    $book = "1";
    $amount = "1320";
    $table_size = "TABLE SIZE 6";
    $stmt = $mysqli->prepare("select * from booking_bhopal_1_6 where date = ? AND timeslot=?");
    ...
    if($stmt->execute()){
        ...
        if($result->num_rows>0){
            ...
            $stmt = $mysqli->prepare("INSERT INTO booking_bhopal_1_6 (name, timeslot, email,phone,name2,name3,name4,name5,name6, date,location,amount,table_size) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?)");
            $stmt->bind_param('sssssssssssss', $name, $timeslot, $email,$phone,$name2,$name3,$name4,$name5,$name6, $date,$book,$amount,$table_size);
            $stmt->execute();

...
st.title(":bar_chart: Rasturant Analysis Dashboard")
...
size = st.sidebar.multiselect(
    "select the size of table varaible ",
    options=df["size"].unique(),
    default=df["size"].unique()
)

day = st.sidebar.multiselect(
    "select the Day varaible",
...
age= st.sidebar.multiselect(
    "select the option of age varaible",
...
foodquality = st.sidebar.multiselect(
    "select the  Food Quality varaible",
...
with left_column:
     st.subheader("Total Amount of income by Bill")
     st.subheader(f"₹ {total_bill:,}")

with right_column:
     st.subheader("Average Amount of income by Bill")
...
left_column ,right_column =st.columns(2)
with left_column:
     st.subheader("Total Amount of income by TIP")
     st.subheader(f"₹ {total_tip:,}")

with right_column:
     st.subheader("Average Amount of income by TIP:")
...
pie_chart_size = px.pie(df_selection, 
                title='chart of  total size of table',
                values='total_bill',
                names='size')
# st.plotly_chart(pie_chart_size)

pie_chart_size_tip = px.pie(df_selection, 
                title='chart of  total tip of  size of table',
                values='tip',
                names='size')
...
pie_chart_age = px.pie(df_selection, 
                title='chart of  total amount by age',
                values='total_bill',
                names='age')
# st.plotly_chart(pie_chart_age)

pie_chart_age_tip = px.pie(df_selection, 
                title='chart of  total tip anount by age',
                values='tip',
                names='age')
